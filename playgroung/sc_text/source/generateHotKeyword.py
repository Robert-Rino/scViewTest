...
subtitle_dir = os.path.join(current_dir,'..','it_done')
timeLine_dir = os.path.join(current_dir,'..','to_weeks_preprocessed')
result_dir = os.path.join(current_dir,'..','hot_word')
...
for week in weekList:
    ...
    week_name = int(week)
    with open(os.path.join(result_dir,str(week_name)+'.txt'), 'w') as outfile:
        json.dump(word_list, outfile, ensure_ascii=False)

    weekly[int(week)] = word_list

with open(os.path.join(result_dir,'tagcloud.txt'), 'w') as outfile:
